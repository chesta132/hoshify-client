...
import { serviceUpdateLink, serviceCreateLink } from "@/services/linkService";
...
export function LinkPopup({ popup, setPopup }: { popup: Popup; setPopup: React.Dispatch<React.SetStateAction<Popup>> }) {
  ...
  const handleCreate = serviceUpdateLink({ formGroup, handlePopup, setLoading });
  const handleEdit = serviceCreateLink({ editId, formGroup, handlePopup, setLoading });
